    ...
    for (const objCombusiness of jsonInput.CommunicationBusinessRules) {
      for (const objActions of objCombusiness.Actions) {
        var configuration: Configurations = {
          ...
          partnerCode: mapCode(
            objCombusiness.BusinessDimensionValues.PartnerCode
          ),
          productCode: mapCode(
            objCombusiness.BusinessDimensionValues.ProductCode
          ),
          businessUnitCode:
            objCombusiness.BusinessDimensionValues.BusinessUnitCode,
...
fs.writeFile("templates.json", JSON.stringify(jsonOutPut, undefined, 2));
